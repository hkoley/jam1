...
c...JFLG=-1, error occurred abandon this event.
...
c...Local values.
     ...
     $,ipa(2,2)
      dimension pjet0(2,5),pc(5),rc(4),p1(5),p2(5)
      dimension indd(100)
...
c...Initialize Pythia.
c     if((kfcol1.ne.kfcol3).or.(kfcol2.ne.kfcol4)) then
        do j=1,5
        pjet(1,j)=pjet0(1,j)
        pjet(2,j)=pjet0(2,j)
        p1(j)=pjet0(1,j)
...
c...Compute beta and phi,theta for the Lorentz transform to the two-body c.m.
      bex=pcm(1)/pcm(4)
      bey=pcm(2)/pcm(4)
      bez=pcm(3)/pcm(4)
      gamcm=pcm(4)/pcm(5)
      call jamrobo(0.0d0,0.0d0,-bex,-bey,-bez,gamcm,
     & p1(1),p1(2),p1(3),p1(4))
      call jamrobo(0.0d0,0.0d0,-bex,-bey,-bez,gamcm,
     & p2(1),p2(2),p2(3),p2(4))
...
c...Rotation so that p_x=p_y=0.
      ...
      call jamrobo(0.0d0,-phi,0.0d0,0.0d0,0.0d0,1.0d0,
     & p1(1),p1(2),p1(3),p1(4))
      call jamrobo(0.0d0,-phi,0.0d0,0.0d0,0.0d0,1.0d0,
     & p2(1),p2(2),p2(3),p2(4))

      theta=pjangl(p1(3),p1(1))
      call jamrobo(-theta,0.d0,0.d0,0.d0,0.d0,1.0d0,
     & p1(1),p1(2),p1(3),p1(4))
      call jamrobo(-theta,0.d0,0.d0,0.d0,0.d0,1.0d0,
     & p2(1),p2(2),p2(3),p2(4))
...
c     print *,'pjet1=',pjet(1,:)
c     print *,'pjet2=',pjet(2,:)
c------------------------------------------------------------------------------
...
c       endif
      ...
      if(isw1.eq.1) then
c       pxp=p(1,jp)-pjet(3,1)
c       pyp=p(2,jp)-pjet(3,2)
c       pzp=p(3,jp)-pjet(3,3)
c       pep=p(4,jp)-pjet(3,4)

c       pxt=p(1,jt)-pjet(4,1)
c       pyt=p(2,jt)-pjet(4,2)
c       pzt=p(3,jt)-pjet(4,3)
c       pet=p(4,jt)-pjet(4,4)

        pxp=pjet(1,1)-pjet(3,1)
        pyp=pjet(1,2)-pjet(3,2)
        pzp=pjet(1,3)-pjet(3,3)
        pep=pjet(1,4)-pjet(3,4)

        pxt=pjet(2,1)-pjet(4,1)
        pyt=pjet(2,2)-pjet(4,2)
        pzt=pjet(2,3)-pjet(4,3)
        pet=pjet(2,4)-pjet(4,4)
...
c...Back to the computational frame.
c     call pjrobo(1,njet,-pare(17),-pare(16),
c    & -pare(12),-pare(13),-pare(14))
      call pjrobo(1,njet,theta,phi,bex,bey,bez)

c     print *,'boost1=',pare(17),pare(16),pare(15),
c    & pare(12),pare(13),pare(14)
c     print *,'boost2=',theta,phi,gamcm,bex,bey,bez
c     read(5,*)
...
c...Check minimum string mass.
      ...
      kf=kf0
      call jamidres(0,kf,pc(5),icon)
      if(icon.ne.0) goto 155

c...Check minimum mass.
      call jamdmass(kf,kfm,kfd,emin,emdn,ist)
...
cxxxxxxxxxxxxxxxxxxxxxxxxxxx
      if(mstc(8).ge.3) then
        write(ih,*)'convert parton system to hadron',kf1,kf2,kf,pc(5)
      endif
cxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
c...Remove collision
      if(mstc(6).ge.0) then
        call jamcupda(jp,-1,0,0,0)
        call jamcupda(jt,-1,0,0,0)
      end if
...
c....Sort index of baryons
          ...
          if(k(1,jp).gt.10) then
            ...
            mdel=0
            call jamindb(indx,ind,0,mdel,idel,1)
            ipd=jp
            if(rn(0).gt.0.5) ipd=jt
            call jamcupda(idel(1),-1,ipd,0,1)
            call jamexch(indx,i)
            ind(j)=indx
          endif
          call jamzero(i)
          ...
          if(k(1,ii).eq.4) then
            ...
            do i=1,nadd
             ...
             if(mstc(6).ge.0) then
               ipd=jp
               if(rn(0).gt.0.5) ipd=jt
               call jamcupda(indd(i),-1,ipd,0,1)
             endif
            end do
          endif
        end do 
...
c...Life time of space-like branch.
      if(is.eq.2) then
          ...
          jamprtim=min(1.0d0,parc(75)*gam/qv*paru(3))
c     else
c     tau=parc(75)/wid*paru(3)
