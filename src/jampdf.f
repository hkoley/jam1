...
C...Call PDFLIB parton distributions.
          ...
          if(mint(93).ne.3000000+mstp(55)) then
            call jampdfset(parm,value)
            mint(93)=3000000+mstp(55)
          endif
          xx=x
          qq=sqrt(max(0d0,q2min,q2))
          if(mstp(57).eq.0) qq=sqrt(q2min)
          call jamstructm(xx,qq,upv,dnv,usea,dsea,str,chm,bot,top,glu)
          vint(231)=q2min
          xpq(0)=glu
          xpq(1)=dnv
          xpq(-1)=dnv
...
C...Dummy routine, to be removed when PDFLIB is to be linked.
      ...
      subroutine jampdfset(parm,value)
...
C...Dummy routine, to be removed when PDFLIB is to be linked.
      ...
      subroutine jamstructm(xx,qq,upv,dnv,usea,dsea,str,chm,bot,top,glu)
