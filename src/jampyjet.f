...
c...jamrqmd
      common/jamvpot/vpot,mstc108,mstc109
      common/jyjets/n,npad,k(1000,5),p(1000,5),v(1000,5)
      save /jyjets/,/jampos1/,/jamvpot/
...
cjamrqmd++
      if(mstc109.ge.2) then
        if(.not.jamrqpb2(k(n+2,2))) then
          if(mstc108.ge.3) then
            p(n+2,5)=p(n+2,5)+vpot
          else
            p(n+2,5)=sqrt(max(0.0001d0,p(n+2,5)**2+2*p(n+2,5)*vpot))
          endif
        endif
        if(.not.jamrqpb2(k(n+3,2))) then
          if(mstc108.ge.3) then
            p(n+3,5)=p(n+3,5)+vpot
          else
            p(n+3,5)=sqrt(max(0.0001d0,p(n+3,5)**2+2*p(n+3,5)*vpot))
          endif
         endif
      endif
cjamrqmd--
...
C...Check flavours and invariant masses in parton systems.
 ...
 800    format('mass=',g13.4,3(1x,g13.4))
...
C...this is usually the case!
  ...
  200 ntry=ntry+1
      if(ntry.gt.50) then
        vpot=0.0d0
      else if(ntry.gt.100.and.ntryr.le.4) then
        paru12=4d0*paru12
        paru13=2d0*paru13
        goto 140
...
cJAM--
      ...
      p(i,5)=pjmass(k(i,2))
cjamrqmd++
      if(mstc109.ge.2.and..not.jamrqpb2(k(i,2))) then
c       kcp=jamcomp(abs(k(i,2)))
c       if(pmas(kcp,2).le.1.d-7) then
          vpot1=vpot
c         if(jamrqpb2(k(i,2))) vpot1=0.0
c         p(i,5)=sqrt(max(0d0,p(i,5)**2+2*p(i,5)*vpot1))
          if(mstc108.ge.2) then
            emfsq=(p(i,5)+vpot1)**2
          else
            emfsq=p(i,5)**2+2*p(i,5)*vpot1
          endif
          if(emfsq.le.0d0) emfsq=0.001d0
          p(i,5)=sqrt(emfsq)
c         print *,'string 1 p5=',k(i,2),p(i,5),vpot1
c       endif
      endif
cjamrqmd--
...
c         if(jamrqpb2(k(i,2))) vpot1=0.0
          if(mstc108.ge.2) then
            ...
            p(i,5)=sqrt(max(0.0001d0,p(i,5)**2+2*p(i,5)*vpot1))
          endif
c         print *,'string 2 p5=',k(i,2),p(i,5),vpot1
c       endif
      endif
cjamrqmd--
...
c         if(mstc108.ge.201) then
c           emfsq=(p(i,5)*(1+vpot1))**2
c         else
c           emfsq=max(0.00001,p(i,5)**2+2*p(i,5)*vpot1)
c         endif
c         p(i,5)=sqrt(emfsq)
c     endif
c....end 2016/4/1
...
c...2014/4/2
c     emfsq=p(i,4)**2-p(i,1)**2-p(i,2)**2-p(i,3)**2
c     if(emfsq.le.0d0) then
c       print *,'pjdec emfsq <0 for decay',emfsq,k(i,2)
c       pot=0d0
c       goto 260
c     endif
c....2014/4/2 end
