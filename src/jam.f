...
c                                                           Aug. 1999  *
c   Many interesting physics are jammed in heavy ion collisions.       *
c   I hope you have a real jam with JAM.                               *
c   JAM is a Monte Carlo program to solve numerically                  *
c   the relativistic Boltzmann equation for hadrons,                   *
...
c  s  jamevol  to compute time evolution of complete one event         *
c  s  jamhybrid do hybrid simulation with hydrodynamics                *
...
c  s  jamdefrm to sample nucleon position for deformed nucleus         *
...
c  s  jamdxw   to add small width (1 MeV) to D*, B*                    *
c  s  jamrmspectator to eliminate predicted spectator nucleons         *
c                                                                      *
c                                                                      *
c***********************************************************************
c***********************************************************************
c                                                                      *
c     Version 0.001  by Yasushi Nara       last revised 1997/10/29     *
c     Version 0.083e by Yasushi Nara       last revised 1998/03/13     *
c     Version 0.085a by Yasushi Nara       last revised 1998/04/07     *
c                             Box calculation by Toshiki Maruyama      *
c     Version 0.088a                       last revised 1998/06/16     *
c                    B-W integral fitted by Chiba is implemented.      *
c     Version 0.090a  many changes         last revised 1998/07/28     *
c     Version 1.004b  buu part             last revised 1998/11/12     *
c     Version 1.005   buu coulomb part     last revised 1998/11/15     *
c     Version 1.009.12 last revised   2/22/2000                        *
c     Version 1.010.01 last revised   2/23/2002                        *
c     Version 1.011.00 last revised   7/17/2003                        *
c     Version 1.07     last revised   4/04/2005                        *
c     Version 1.08     last revised   4/09/2005                        *
c     Version 1.09     last revised   8/14/2005                        *
c     Version 1.30     last revised  11/13/2014                        *
c     Version 1.32     last revised  12/05/2014                        *
c     Version 1.44     last revised  08/04/2015                        *
c                                                                      *
c***********************************************************************
...
c...Rest collision counters.
      ...
      mstd(54)=0   ! Econ block -> RR -> NN collision 2020/8/25
...
c...Start time evolution.
      if(mstc(140).eq.0) then
        call jamevol(npri,nann,icon)
      else
c       if(mstc(141).le.1) then
c         call jamhybrid1(npri,nann,icon)
c       else
          call jamhybrid(npri,nann,icon)
c       endif
      endif
      if(icon.eq.3000) goto 3000
...
c...Transport particles that are still within a formation time.
      do ip=1,nv
        ...
        do j=1,4
          v(j,ip)=r(j,ip) ! bug fix 2017/5/7
        end do
      end do
...
c...2014/1/27 add mstc(18)
      if(mstc(18).eq.0.and.mstd(30).gt.0) call jamedit
c     if(mstc(18).eq.0.and.mstd(30).gt.0) then
c       call jamedit
c       write(mstc(38),*)'jamedit? mstc18 mstd30',mstc(18),mstd(30)
c     endif

c...Nuclear cluster formation.
      if(mstc(131).ge.1) call jamclusterform
...
c...Update collision counters.
      ...
      pard(84)=pard(84)+mstd(54)   ! RR->NN coll.
      ...
      if(mstc(8).ge.3) then
        ...
        write(ih,*)'RR->NN collisions',mstd(54)/mstc(5)
...
c**********************************************************************
      ...
      subroutine jamhybrid1(npri,nann,icon)

c...Purpose: to compute time evolution of hybrid simulation with hydrodynamics.

      implicit none
      integer npri,nann,icon,kdt,icheck
      integer nstep
      real*8 tmax,dt
      include 'jam2.inc'

c...Reset time dependent analysis.
c     if(mstc(3).eq.1) then
c...Option: output particles on the terminal.
c      if(mstc(8).ne.0) call jamdisp(6,1)
c       npri=1
c       nann=1
...
c     endif
      ...
      mste(41)=0   ! flag for hydro simulation.
c...Hydro conversion time.
c...Initial time step for hydro simulation
      mstd(140)=int(pard(141)/parc(2))

c...Reset all fluid variables.
      call reset_fluid_val

c...Set final time for search for collision to be hydro switching time.
      mstd(23)=0
      mstc(3)=1
      pard(1)=0d0  ! Initial time
      pare(1)=pard(141)  ! Final time for collision.
      pare(10)=pard(141) ! Final time for propagation.
      pard(20)=pard(141)
...
c      print *,'before collision pard(141)',pard(141)

c...Perform collisions or decay of particles.
      mstd(20)=0
      call jamcoll
...
c...Perform hydro conversion at once.
      print *,'Now switch to hydro',pard(1),pard(141)
      call jam2hydroall(pard(1))
      mste(41)=1   ! flag for hydro simulation.
      if(mstc(144).ge.1) call c_freezeout(1,0,0d0)

      mstc(3)=nstep

c...Start hydro time evolution.
c======================================================================*
      do 10000 kdt=0,nstep
c======================================================================*

        mstd(23)=kdt

c...Set current time.
        pard(1)=kdt*dt + pard(141)

c...Set max. time for cascading.
        pare(1)=(kdt+1)*dt + pard(141)
        pare(10)=pare(1)

c...Fluid particle interaction.
       if(mstc(143).ge.1) call jam2fluid_dynamical3

c...Print information.
        if(mstc(8).ge.1) then
          if(nstep.ge.2.and.mod(kdt,npri).eq.0) then
            call jamdisp(6,kdt)
          endif
        endif

c...Time dependent analysis.
        if(nstep.ge.2.and.mod(kdt,nann).eq.0) then
           if(mstc(161).ge.1) call jamana(2)
        endif

c...Perform time evolution of fluid.
        call fluid_evolution1(mstd(140),icheck)
        mstd(140)=mstd(140)+1
        if(pard(1).gt.pard(140).and.icheck.eq.0) exit

c======================================================================*
10000 continue  !******************** End time evolution
c======================================================================*

      if(mstc(144).eq.0) return
      ...
      if(mstc(144).eq.1) call isochronous_freezeout(pard(1),0)
      call hydro2jamall(mstd(140),0)
      mstd(30)=1
      call jamedit
      mste(41)=0   ! flag for hydro simulation.

      if(mstc(8).ge.1) call jamdisp(6,0)
...
c**********************************************************************
      ...
      subroutine jamhybrid(npri,nann,icon)

c...Purpose: to compute time evolution of hybrid simulation with hydrodynamics.

      implicit none
      integer npri,nann,icon,inithydro,ihsw,kdt,icheck,iopt_qmd
      parameter(iopt_qmd=0)
      real*8 dt
      include 'jam2.inc'

      if(mstc(6).lt.0) then
        print *,'jamhybridevolution invalid mstc(6)',mstc(6)
        stop
      endif

c...Compute mumber of collision and participants in the Glauber theory.
      call jamglaub(icon,0)

      icon=0

      mstd(140)=0  ! Current time step for hydro simulation
      mste(41)=0   ! flag for hydro simulation.
      dt=parc(2) 
...
c...Set final time for search for collision.
c     pare(1)=mstc(3)*dt
c     pard(20)=pare(1)
c..Make initial collision list. Because of this call, parallel ensumble
c..method cannot be sued.
c     if(mstc(6).eq.0) then
c       mstd(8)=1
c       call jamclist
c      endif

c...Hydro conversion time.
      if(mstc(141).le.2) then
        inithydro=0
        ! Switching time is the last Glauber type BB collision
        if(mstc(141).eq.2) then
          ihsw=int(pard(23)/parc(2))
          write(6,*)'switching time=',pard(23)
          flush(6)
        else
          ihsw=int(pard(141)/parc(2))
        endif
c       time_fluid_start=pard(23)

c...Dynamical conversion.
      else
        ihsw= -1
        inithydro=mstc(141)
        if(mstc(144).ge.1) call c_freezeout(1,0,0d0)
        call fluid_initialtime(0.0d0)!This should be modified for tau-eta
c       time_fluid_start=0.0
      endif


c...Start time evolution.
c======================================================================*
      do 10000 kdt=0,mstc(3)
c======================================================================*
...
c...Set max. time for QMD mode.
c       if(mstc(6).gt.0) then
c       pare(1)=(mstd(23)+1)*parc(2)*mstc(7)
c       pard(20)=pare(1)
c       endif

c...Set next time for transport.
        pare(10)=(kdt+1)*dt
c       pare(1)=pare(10)*mstc(7)
        pare(1)=max(parc(126),pare(10)*mstc(7))
        pard(20)=pard(1)

c...Perform hydro conversion at once.
        if(inithydro.eq.0 .and. kdt.eq.ihsw) then 
          call jam2hydroall(pard(1))
          if(mstc(144).ge.1) call c_freezeout(1,0,0d0)
        endif

c...Some of particles are converted into fluid dynamically.
        if(mste(41).ne.-1.and.inithydro.ge.3) then
          if(mstc(141).eq.4) call jam2fluid_dynamical1
          if(mstc(141).eq.5) call jam2fluid_dynamical2
        endif

c...Fluid particle interaction.
        if(mste(41).ge.0) then
          if(mstc(143).ge.1) call jam2fluid_dynamical3
        endif

c...Print information.
        if(mstc(3).ge.2.and.mod(kdt,npri).eq.0) then
            if(mstc(8).ge.1) then
              call jamdisp(6,kdt)
            endif
        endif
...
c...Time dependent analysis.
        if(mstc(3).ge.2.and.mod(kdt,nann).eq.0) then
           if(mstc(161).ge.1) call jamana(2)
        endif

c...Output density or fluid fraction.
        if(mstc(181).ge.1) call dens0output(kdt,dt,1)
        if(mstc(182).ge.1) call hydrofraction_output(kdt,dt,1)

c...Perform time evolution of fluid in case mste(41).ne.0 which means that
c...there are fluid elements.
        if(mste(41).ge.1) then
          call fluid_evolution1(kdt,icheck)
          mstd(140)=mstd(140)+1
          if(ihsw.eq.-1) ihsw=kdt

c......mstc(144)>=3: Dynamical freezeout.
c        if(mstc(144).ge.3.and.pard(1).gt.pard(140)) then
         if(mstc(144).ge.3) then
           call hydro2jamall(mstd(140),icheck)
           if(icheck.eq.0) then
              call reset_fluid_val
              mste(41)=-1
              mstd(30)=1
...
c....Perform freeze-out; Convert all fluid to JAM particles.
c         if(pard(1).gt.pard(140).and.icheck.eq.0) then
          if(icheck.eq.0) then
c         if(icheck.eq.0.or.kdt.eq.mstc(3)) then
            if(mstc(144).eq.0) exit
            if(mstc(144).eq.1) call isochronous_freezeout(pard(1),0)
            call hydro2jamall(mstd(140),icheck)
            if(mstc(181).ge.1) call dens0output(kdt,dt,2)
            call reset_fluid_val
            mste(41)=-1
            mstd(30)=1
            call jamedit
            if(mstc(151).eq.0) exit  ! No hadron cascade after burner
            call jamclist
            if(iopt_qmd.eq.1.and.mstc(6).ge.1) goto 4000 ! QMD mean-field on.
          endif
         endif
        endif

c...Collision list will be update or not.
      mstd(20)=1   ! Default: collision list will not be updated.
c...In case of QMD, collision list should be updated.
c     if(mstc(7).ge.1.and.mstc(6).gt.0) then
      if(mstc(7).ge.1) then
        mstd(20)=mod(kdt,mstc(7))
      endif
c...Perform collisions or decay of particles.
        call jamcoll
...
c======================================================================*
      ...
      return
c     if(mstc(144).ge.3.or.mstc(6).eq.0) return

4000  continue
      ihsw=max(0,ihsw)
c...Start time evolution for QMD.
      do kdt=ihsw,mstc(3)

        mstd(23)=kdt
        pard(1)=kdt*dt               ! Set current time.
        pare(10)=(mstd(23)+1)*parc(2) ! Set max. time for QMD mode.
c       pare(1)=pare(10)*mstc(7)
        pare(1)=max(parc(126),pare(10)*mstc(7))
        pard(20)=pard(1)
        mstd(20)=1   ! Default: collision list will not be updated.
        if(mstc(7).ge.1.and.mstc(6).gt.0) mstd(20)=mod(kdt,mstc(7))
        call jamcoll
...
c**********************************************************************
      ...
      subroutine jamevol(npri,nann,icon)

c...Purpose: to compute time evolution of complete one event.

      implicit double precision(a-h, o-z)
      include 'jam2.inc'

      pard(20)=parc(2)*mstc(7)
...
c...Start time evolution.
c======================================================================*
      do 10000 kdt=0,mstc(3)-1
c======================================================================*

        mstd(23)=kdt
c...Current global time.
        pard(1)=mstd(23)*parc(2)

c...Set max. time for cascading.
c       pare(1)=(mstd(23)+1)*parc(2)
c       pare(10)=pare(1)
c       pard(20)=pare(1)

c...Max. time to transport particles within a time step.
        pare(10)=(mstd(23)+1)*parc(2)

c...Set max. time for collision search.
        pare(1)=max(parc(126),pare(10)*mstc(7))
c       pare(1)=pare(10)*mstc(7)
        pard(20)=pard(1)
...
c...Collision list will be update or not.
          mstd(20)=1
          if(mstc(7).ge.1) mstd(20)=mod(kdt,mstc(7))
          call jamcoll
...
c...Multiple AA collision by Glauber geometry.
        else
          call jamglaub(igl,1)
          if(igl.eq.1) then
            icon=3000
...
c...Option for final state hadron cascade.
          if(mstc(6).lt.0.and.mod(abs(mstc(6))/10,10).eq.2) then
            mstc55=mstc(55)
...
c...CPU time and initialization of random number.
c       call jamcpu(6,0,mstc(1),isec)
...
c...Initialize Multi run control
...
c...Initialize event study and analysis.
      if(mstc(161).ge.1) call jamana(0)

c...Initialize soft and hard interaction part and string fragmentation.
      call jamjeti
...
crqmd...Add small width (1 MeV) to D*, B*  why?
      call jamdxw

c...Parameters for mstc(71)=1 ! original diffractive scattering
      if(mstc(71).eq.1) then
        mstc(61)=2
        mstc(74)=0   !QCD dipole radiation switched off
        parc(64)=1.1
      endif


c....In case time step is greater than 1.
c     if(mstc(3).ge.2) mstc(52)=5

c...rqmd mode
c     if (mstc(6).ge.101) then
c       if(mstc(98).eq.1) then
c         print *,'error in QMD mode mstc(98) should be 2 or 3',mstc(98)
c       endif
c     endif

c...In the case of mean field, collision time of each particle
c...is set the same as collision time.
c     mstc(52)=5
      
c...Set potential parameters (jamrqmd.f)
      call qmdpotparam

c...mstc(107) is not used anymore?
c     if(mstc(6).le.100) then
c       if(mstc(107).ge.1) then
c          write(6,*)'switch inconsistent mstc106 mstc107',
c    &           mstc(6),mstc(107)
c          mstc(107)=0
c       endif

c       if(mstc(109).ge.2.and.mstc(49).le.2) then
c          write(6,*)'switch inconsistent mstc106 mstc109',
c    &           mstc(6),mstc(109)
c          mstc(109)=0
c       endif

c...The probability for QCD dipole radiation.
c     if(mstc(80).eq.0) then
      hidat0(3,1)=0.3
      hidat0(3,2)=0.3
c     endif
...
c...factor for the Gaussian.
      pard(90)=(1.d0/(3.14159d0*2*parc(84)**2))**1.5d0

c...Reading EoS file.
      if(mstc(50).ge.21) then
        iopt=mod(mstc(50),10)
        print *,'EoS iopt=',iopt,' reading file'
        call readeos(iopt,fname(9))
      else if(mstc(50).ge.11) then
        print *,'EoS mstc(50)',mstc(50),' reading file'
        call readEOStable(fname(9))
      endif

c...Initialize hydro part
      if(mstc(140).ge.1) then
        if(mstc(3).le.10) then
          print *,'hydro simulation: time step is enough?',mstc(3)
          stop
        endif

! full ensamble method. Parallel ensamble method does not make ! sens in hydro.
        mstc(46)=2

c...Set switching time to hydro.
      pard(141)=parc(141)
c...Compute passage time of two nuclei.
      gam1=pard(36)
      gam2=pard(46)
      beta1=pard(35)
      beta2=pard(45)
c     rat1=dble(mstd(5))/dble(mstd(5)+mstd(2))
c     rat2=dble(mstd(2))/dble(mstd(5)+mstd(2))
      r1=0d0
      r2=0d0
      if(mstd(2).gt.2) then
        r1=1.19d0*dble(mstd(2))**(1.d0/3.d0)
     $        - 1.61d0*dble(mstd(2))**(-1.d0/3.d0)
      endif
      if(mstd(5).gt.2) then
        r2=1.19d0*dble(mstd(5))**(1.d0/3.d0)
     $        - 1.61d0*dble(mstd(5))**(-1.d0/3.d0)
      endif

c     pard(140)=( pard(50)/gam2 + pard(40)/gam1 + pard(49) - pard(39) )
c    &      /(beta1-beta2)
      pard(140)=( 2*r2/gam2 + 2*r1/gam1 + 2*parc(5) )/(beta1-beta2)
      if(mstc(141).eq.0) pard(141)=pard(140)
      
      write(6,800)pard(140),pard(141)
c     print *,'r=',pard(50),pard(40),r1,r2
800   format('passage time=',f8.4,' switching time=',f8.4)
...
c     else if( mstc(7) .le. 30 ) then
      endif

c....Eliminate predicted spectator nucleons from collision zone.
      if(mstc(34).eq.1) call jamrmspectator
...
c...Save total momentum, charge, strangeness, and baryon number.
      ...
      if(mstc(8).ge.1) write(6,800)pard(9),pard(10),pard(11)
800   format('initial total momentum',3(e12.5,1x))
...
crqmd...RQMD/S by Hirata-Isse
      else if(mstc(6).ge.100) then
        call jamrqmde(ekin,epot,etot)
        pard(12)=etot*mstd(11)
        pard(13)=etot
...
c...Default values.
      ...
      pard(20)=100.d0    ! Time step size(fm/c)
...
c...Check consistency of swishes
...
crqmd....Check number of test particles.
      if(mstc(6).ge.100) then ! RQMD/S:test particle/hardon = 1 is allowed.
...
c...In the case of Proj. number =0
...
c        pard(39)=-(pard(40)/gam1 + parc(5)*rat1)
c        pard(49)=pard(50)/gam2 + parc(5)*rat2

         pard(39)=-(pard(40)+1.0)/gam1 - parc(5)
         pard(49)=(pard(50)+1.0)/gam2 + parc(5)
        ...
        if(inuc(i).eq.0) then
          ...
          if(mstd(i*3-2).eq.-9999) then
            ...
            if(mstd(i*3-2).eq.0) then
              if(i.eq.1) write(6,5000) chbeam(1:len(1))
              if(i.eq.2) write(6,5100) chbeam(1:len(2))
              stop
            endif
          endif
...
c....Check if proj. and targ. are defined.
      if(mstd(1).eq.0.or.mstd(4).eq.0) then
...
c...Collider   elab means c.m. energy per nucl.
     ...
     $ .or.chcom(3)(1:4).eq.'user') then
        ...
        srt=abs(elab)
        s2=srt*srt
c       elab=(s2- pm(1)**2-pm(2)**2)/(2*pm(2))-pm(1)
        elab=(s2- pm(1)**2-pm(2)**2)/(2*pm(2))
        p01=sqrt((s2-(pm(1)+pm(2))**2)*(s2-(pm(1)-pm(2))**2))
     $             /(2*srt)
        p02=-p01
        e1=sqrt(pm(1)**2+p01**2)
...
c        pproj=p01
c        ee=sqrt(pm(1)**2+pproj**2)
c        ee=elab+pm(1)+pm(2)
         pproj=sqrt(elab**2-pm(1)**2)
         ee=elab
         dbeta=pproj/ee
...
C...Unknown frame. Error for too low CM energy.
      ...
      pard(16)=srt
      pard(17)=ylab
      pard(19)=pawt(srt,pm(1),pm(2))
      dbeta=pard(19)/sqrt(pard(19)**2+pm(1)**2)
      pard(18)=0.5d0*log((1.d0+dbeta)/(1.d0-dbeta))
...
c...Local arrays and character variables.
     ...
     &vers*1, subv*3, date*2, year*4, hour*2, minu*2, seco*2,aamode*20
...
c...Write time step,event, .....
      ...
      if(mstc(6).eq.0) then
        aamode='Cascade mode'
      else if(mstc(6).eq.101) then
        aamode='RQMD/S mode'
      else if(mstc(6).ge.2.and.mstc(6).le.20) then
        aamode='BUU mode'
      endif
      write(io,617)mstc(6),aamode,mstc(106)
 617  format(
     &       6x,'* mstc(6) = ',i5
     &      ,1x,'Simulation mode = ',a13,' mstc(106)=',i3/)
...
c---------------------------------------------------------------
 ...
 909  format(6x,'*',10x,'ylab yproj:',5x,f9.3,5x,f9.3,16x,'*')
...
c...Write first few errors, then be silent or stop program.
      elseif(merr.le.20) then
        ...
        if(mstc(11).eq.2) then
          print *,'jamerrm merr mstc(11)=2'
          stop
        endif
...
901     format(/,8x,'* CPU time=',i4,' h ',i3,' m ',i3,' s')
        write(mstc(37),'(8x,31(''*''))')
        close(mstc(37))
        print *,'jamerrm stop because of error'
        stop
...
c...Purpose: to list event record or particle data.
      ...
      include 'jam1.inc'
      include 'jam2.inc'
      character chau*16,chap*16,chan*16

      io=mstc(38)
      iev=mstd(21)   ! Added by T. Kamae
...
c      else if(mlist.eq.4) then
c
c      write(io,803)
c      do i=1,nv
c        write(io,927)  (r(l,i),l=1,5),(p(l,i),l=1,5),(v(l,i),l=1,5),
c     &                 (k(l,i),l=1,9)
c      end do

      else if(mlist.eq.4) then   ! Added by T. Kamae

      write(io,804) iev
      do i=1,nv
        write(io,928)  iev, i, (p(l,i),l=1,5), k(2,i), k(4,i), k(7,i),
     &                 k(9,i)
      end do

      else if(mlist.eq.5) then

      write(io,804) iev
      do i=1,nv
        write(io,929)  iev, i, (r(l,i),l=1,5), k(1,i), k(2,i), k(7,i)
      end do

      else if(mlist.eq.11) then
        write(6,6600)
        do kc=1,500
          kf=kchg(kc,4)
          call pjname(kf,chap)
          call pjname(-kf,chan)
          if(chap.ne.' '.and.chan.eq.' ') write(6,6700)kc, kf,chap
          if(chan.ne.' ') write(6,6700)kc, kf,chap,-kf,chan
        end do
      endif
 6600 format(///20x,'List of KF codes in program'/)
 6700 format(4x,i3,2x,i9,4x,a16,6x,i9,4x,a16)

 800  format(//,6x,'Event Listing of JAM records 1 ch=',i2)
 ...
 804  format(//,1x,'#Event No.= ',i6, ': p1, p2, p3, E, mass; KFcode,' 
     &                          ' KFances, NoColl(proj+,targ-), 3xB') 
...
c927  format(3d13.5,f8.4,1x,d13.5,1x,5f8.4,4f7.4,d11.5,i3,i6,20i5)
c 911  format(i3,1x,i5,1x,8(i3,1x),i5,1x,3d13.5)
c 912  format(1x,a9,i3,1x,i5,1x,2(i3,1x),i7,1x,3d13.5)
 928  format(3x,i6,1x,i2,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,
     & i7,1x,i7,1x,i3,1x,i2)
 929  format(3x,i6,1x,i5,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,f8.3,1x,
     & i7,1x,i7,1x,i3,1x,i2)
...
c...Box:maru
      ...
      save etot0
      save ncl0,ndec0
      data etot0/0.0d0/,ifirst/0/,ncl0/0/,ndec0/0/
      dimension ptot(3)
      ...
      do i=1,nv
        if((k(1,i).ge.1.and.k(1,i).le.10).or.k(1,i).lt.0) then
          if(k(2,i).eq.92) then
            ptot(1)=ptot(1)+vq(1,i)+vq(6,i)
            ptot(2)=ptot(2)+vq(2,i)+vq(7,i)
            ptot(3)=ptot(3)+vq(3,i)+vq(8,i)
...
crqmd
      if(mstc(6).ge.100) then
        ! Sorry current cpu version does not include meson potentials.
        if(mstc(104).ne.13) then
        call jamrqmde(ekin,epot,etot)
        endif
      else  if(mstc(6).ge.2) then
        call jambuue(ekin,epot,etot)
...
c...hydro
      if(mste(41).ge.1) then
        it=mstd(140)
        call hydroTotalEnergy(it,pard(1),etoth,pxh,pyh,pzh,btoth)
        etot=etot+etoth/mstd(11)
        ptot(1)=ptot(1)+pxh
        ptot(2)=ptot(2)+pyh
        ptot(3)=ptot(3)+pzh
      endif

      if(kdt.eq.0) ifirst=0
      if(ifirst.eq.0) then
        etot0=etot
        ncl0=0
        ndec0=0
      ...
      econ=abs(etot-etot0)/max(1d0,etot0)*100
      ene0=abs(etot-etot0)*1000

      ncl=(mstd(41)+mstd(42)+mstd(43)+mstd(50))/mstc(5)
      ndec=mstd(50)/mstc(5)
      ncl2=ncl-ncl0
      ndec2=ndec-ndec0
      ncl0=ncl
...
c     print *,'etot0=',etot0,'etot=',etot,'kdt=',kdt,'ifirst=',ifirst
c     print *,'ekin=',ekin,'nv=',nv,'etoth=',etoth

      write(irec,810)pard(1),ene0,econ,
     $    nbar/mstc(5),nmes/mstc(5),ncl2,ndec2
810   format(f6.2,'(fm/c) econ(MeV/A)=',f9.4,' econ(%)=',f8.4,
     $  ' B',i3,' M',i8,' C',i6,' D',i4)

      write(irec,811)ptot(1),ptot(2),ptot(3)
811   format('total mom.= ',3(e15.8,1x))
...
c     print *,mstd(41),mstd(42),mstd(43),mstd(50),mstc(5)
...
C...Find minimum and maximum z-position of nucleons.
      rzp=-100d0 
      rzt=100d0
c     do i=1,nv
c       if(i.le.mstd(5)*mstc(5)) then
c         rzt=min(rzt,r(3,i)/pard(46)+pard(49))
c       else
c         rzp=max(rzp,r(3,i)/pard(36)+pard(39))
c       endif
c     end do
c     if(rzp.le.0d0) rzp=0d0
c     if(rzt.ge.0d0) rzt=0d0

      do i=1,mstd(5)*mstc(5)
         rzt=min(rzt,r(3,i))
      end do
      do i=mstd(5)*mstc(5)+1,nv
         rzp=max(rzp,r(3,i))
      end do
      if(rzt + pard(49) < 0.0) then
        rzt = abs(rzt)/pard(46) + parc(5)
      else
        rzt = pard(49)
      endif
      if(rzp + pard(39) > 0.0) then
        rzp = -abs(rzp)/pard(36) - parc(5)
      else
        rzp = pard(39)
      endif

c...simulation in the lab. frame
      if(mstc(4)==0) then
        rzp= rzp - abs(rzt)
        rzt=0.0
      endif
...
c...bug fix 2017/5/6
        ...
        ep=1.0d0
c       if(mstc(47).eq.1) then
c         ep=p(4,i)/p(5,i)
c       endif

        p(3,i)=p(3,i)*gmb + pzb*ep
        p(4,i)=sqrt(p(5,i)**2+p(1,i)**2+p(2,i)**2+p(3,i)**2)

c       if(ip.eq.1) then
c          if(r(3,i).lt.rzp) then
c            rzp=r(3,i)
c            vzp=p(3,i)/p(4,i)
c          endif
c       else
c          if(r(3,i).gt.rzt) then
c            rzt=r(3,i)
c            vzt=p(3,i)/p(4,i)
c          endif
...
c     pard(140)=(rzt-rzp)/(vzp-vzt)
c     if(mstc(141).eq.0) pard(141)=pard(140)
c     print *,'pard140=',pard(140)
...
c...HIJING parameterization does not take into account hard core.
      if(mstc(98).eq.0) then
        parc(13)=0.4
        parc(14)=0.4
      endif
...
c....Projectile loop.
       ...
       a3=nnn**(1.0/3.0)
       npro=nnp      ! number of protons
       nneu=nnn-nnp  ! number of neutrons
...
c...Woods-Saxon R0
      if(mstc(4).ne.10) then
        if(mstc(98).eq.1) then
        if(dcore.eq.0d0) then
c....rho0=0.16
c       rad0=1.19d0* nnn**0.333333d0-1.61d0* nnn**(-0.333333d0)-parc(20)
c....rho0=0.17
          rad0=1.12d0*a3-0.86d0/a3-parc(20)
c...GLISSANOModel
        else
          rad0=1.1d0*a3-0.656d0/a3-parc(20)
          parc(11)=0.459
        endif
        radm = rad0+2.5d0
c2018/11/23
        else if(mstc(98).eq.2) then ! JQMD Niita PRC52(1995)2620
         radm=1.124*a3
         rad0=radm - 0.5
         parc(11)=0.2
        elseif(mstc(98).eq.3) then ! UrQMD potential mode
         r0 = (0.75/paru(1)/0.16)**(1./3.)
         rad0= r0*(0.5*(nnn + (nnn**(1./3.)-1.)**3.))**(1./3.)
         radm = rad0
        elseif(mstc(98).eq.4) then ! QMD L=1.0 fm^2
          parc(20)=0.03
          rad0=parc(12)*a3-parc(20)
          radm = rad0+2.5d0
        elseif(mstc(98).eq.5) then ! QMD L=0.5 fm^2
          parc(20)=-0.089
          rad0=1.1d0*a3-0.656d0/a3-parc(20)
          parc(11)=0.35
        else
        ...
        rwmax=1.0d0/(1.0d0+exp(-rad0/parc(11)))
      endif
...
c...Sample particle.
        if(mstc(4).ne.10) then
        if(mstc(97).eq.0) then
          rr=0.0d0
          if(nnn.gt.1) then

            if(mstc(98).eq.0) then ! HIJING parametrization
              if(in.eq.1) rr=hirnd(2)
              if(in.eq.2) rr=hirnd(1)

c...Woods-Saxon
            else if(mstc(98).le.2.or.mstc(98).ge.5) then
c             rr = radm*rn(0)**(1d0/3d0)
c             if(rn(0)*rwmax.gt.1d0/(1d0+exp((rr-rad0)/parc(11))))
c    &             goto 10
              rr=getrnucl(rad0,parc(11))

c...Hard sphere.
            else
              rr = rad0*rn(0)**(1d0/3d0)
            endif
...
c       if(iee.le.nnp) then
c         if(nnn.eq.1) then
c          if(in.eq.2) then
c            kf=mstd(1)
c            kc=jamcomp(kf)
c            p(5,i)=pard(34)
c          else if(in.eq.1) then
c            kf=mstd(4)
c            kc=jamcomp(kf)
c            p(5,i)=pard(44)
c          endif
...
c....Compute density of the nucleus.
      if(mstc(43).eq.2) then
        rhog=0.0d0
        rho=0.0d0
        vmoms=0.0
        do ie=1,nnn*mstc(5)
          i=iofset+ie
          iz1=0
          if(k(2,i).eq.2212) iz1=1
          do je=ie+1,nnn*mstc(5)
            j=iofset+je
            expa =(r(1,i)-r(1,j))**2
     &           +(r(2,i)-r(2,j))**2
     &           +(r(3,i)-r(3,j))**2
     ...
     &         +(p(3,i)-p(3,j))**2
           ...
           den=exp(-expa/(4*parc(15)))/facp
           fac1 = 1.d0 + pp2/pmu1
           fac2 = 1.d0 + pp2/pmu2
           pmom = vex1/fac1 + vex2/fac2
           vmoms(i)=vmoms(i)+pmom*den
           vmoms(j)=vmoms(j)+pmom*den
           rho(i)=rho(i)+den
           rho(j)=rho(j)+den

c...proton or neutron density.
            if(k(2,i).eq.k(2,j)) then
            den=exp(-expa/2.0d0/parc(15))/fac
            rhog(i)=rhog(i)+den
            rhog(j)=rhog(j)+den
            endif
            ...
            ! Coulomb
            iz2=0
            if(k(2,j).eq.2212) iz2=1
            rhocpij = 0.5*parc(23)*iz1*iz2
     &          *erf(sqrt(expa/4/parc(15)))/sqrt(expa)
            rhoc(i) = rhoc(i) +  rhocpij
            rhoc(j) = rhoc(j) +  rhocpij
            cls  = parc(100)/(2.0*parc(21))       ! for symmetry potential
            sym=(1.0-2.0*abs(iz1-iz2))*den
...
c....Sampling (Fermi) momenta of nucleus.
...
c...Calculate local Fermi momentum.
          if(mstc(43).ne.0) then
            if(mstc(43).eq.1) then
            if(in.eq.1) then
              dens=wdsax2(sqrt(r(1,i)**2+r(2,i)**2+r(3,i)**2))*nnn
            else
              dens=wdsax1(sqrt(r(1,i)**2+r(2,i)**2+r(3,i)**2))*nnn
            endif
            else if(mstc(43).eq.2) then
              dens=rhog(i)/mstc(5)
            endif
c           pfermi=paru(3)*(1.5d0*paru(1)**2*dens)**(1.0d0/3.0d0)
            pfermi=paru(3)*(3.0d0*paru(1)**2*dens)**(1.0d0/3.0d0)
...
c     nnr=0
      ...
      do 301 ie=1,nnn*mstc(5)
        ...
        p(3,i)=p(3,i)+cz
        p(4,i)=sqrt(p(5,i)**2+p(1,i)**2+p(2,i)**2+p(3,i)**2)
c       if(k(2,i).eq.2212) npr=npr+1
c       if(k(2,i).eq.2112) nnr=nnr+1
 301  continue

c       print *,'np=',npr,nnr,npr+nnr

c...Check ground state energy.
      if(mstc(96).ge.1)then
        call jamgrenergy(nnn,nnp,iofset,iok)
        if(ktry.ge.100) then
          print *,'gound state cannot be constructed for mstc(96)=',
     &        mstc(96)
...
c...Output  ground state density.
c       call jamgrout(nnn,nnp,iofset)
c       write(10,8000)i,k(2,i),k(8,i),chaf(jamcomp(k(2,i)),1)
c    $  ,(r(j,i),j=1,5),(p(j,i),j=1,5)
...
c     if(mstc(6).ge.100) then
c       call jamrqmm(0)
c       if(mstc(109).ge.1) call jamepart
c     endif
...
c***********************************************************************
      real*8 function getrnucl(ra,a)
      implicit none
      real*8 intlo, inthi0,inthi1,inthi2
      real*8 sel,r,ra,a,rn
      integer i
      ...
      do while(i<1000)
        r=ra
        sel = rn(0)*(intlo + inthi0 + inthi1 + inthi2)
        if ( sel > intlo ) r = r - a*log(rn(0))
        if ( sel > intlo + inthi0 ) r = r - a*log(rn(0))
        if ( sel > intlo + inthi0 + inthi1 )  r = r - a*log(rn(0))
        if ( sel <= intlo ) then
          r = ra*rn(0)**(1.0/3.0)
          if ( rn(0)*(1.0 + exp((r - ra)/a)) < 1.0 ) exit
        else 
          if(rn(0)*(1.0 + exp((r - ra)/a)) < exp((r - ra)/a) ) exit
        endif
        i=i+1
      end do
      getrnucl=r
...
c***********************************************************************
      subroutine jamgrenergy(nnn,nnp,iofset,iok)
c...Compute ground state energy of a nucleus.
      implicit double precision(a-h, o-z)
      include 'jam1.inc'
      include 'jam2.inc'
      include 'jam3.inc'
      ...
      do ie=1,nnn*mstc(5)
        i=iofset+ie
        etot = etot + (p(1,i)**2+p(2,i)**2+p(3,i)**2)/(2*p(5,i))
        etot = etot + t1*rho(i)+t3*rho(i)**pard(103)
        if(mstc(101).eq.1) etot = etot + rhoc(i)
        if(mstc(100).eq.1) etot = etot + rhos(i)
        if(mstd(101).eq.1) etot = etot + vmoms(i)
      end do

      ebin = -15.56*nnn + 17.23*nnn**(2.0/3.0)
      if(mstc(101).eq.1) ebin=ebin
     &      + 3.0/5.0*nnp**2*parc(23)*1000/(1.24*nnn*(1.0/3.0))
      if(mstc(100).eq.1) ebin=ebin + 46.57*(nnn-2*nnp)**2/(2*nnn)

c     ebin = -15.56*nnn + 17.23*nnn**(2.0/3.0)
c    &     + 46.57*(nnn-2*nnp)**2/(2*nnn)
c    &     + 3.0/5.0*nnp**2*parc(23)*1000/(1.24*nnn*(1.0/3.0))

      edef=abs(ebin-etot*1000)/nnn
      iok=0
      if(edef.lt.0.5) iok=1

c     print *,'ebin=',ebin/nnn,etot*1000/nnn,edef

      end

c***********************************************************************
      subroutine jamgrout(nnn,nnp,iofset)
c...Output ground state of a nucleus.
      implicit double precision(a-h, o-z)
      include 'jam1.inc'
      include 'jam2.inc'
      include 'jam3.inc'
      parameter(nhist=80, rmax=15.0, pmax=2.0)
      dimension rhor(nhist),rhop(nhist)
      dimension histr(nhist),histp(nhist)
      save histr,histp,rhor,rhop
      save dr,dp,widr,widp,facr,facp
      save nev
      data nev/0/
      ...
      widr=2*parc(15)
      widp=1.0/(2*parc(15))
      facr=(paru(1)*widr)**(1.5)
      facp=(paru(1)*widp)**(1.5)
      endif
      nev=nev+1
      hc=paru(3)
      ...
      do ie=1,nnn*mstc(5)
        ...
        pp=sqrt(p(1,i)**2+p(2,i)**2+p(3,i)**2)/hc
        ir= nint(rr/dr+0.5)
        ip= min(nhist,nint(pp/dp+0.5))
        histr(ir)=histr(ir)+1/(4*paru(1)*rr*rr*dr)
        histp(ip)=histp(ip)+1/(4*paru(1)*pp*pp*dp)
        do l=1,nhist
          x=l*dr
          px=l*dp
          rx=(r(1,i)-x)**2+r(2,i)**2+r(3,i)**2
          px=(p(1,i)-px*hc)**2+p(2,i)**2+p(3,i)**2
          rhor(l)=rhor(l)+exp(-rx/widr)/facr
          rhop(l)=rhop(l)+exp(-px/widp/hc/hc)/facp
        end do
      end do
...
c       x=l*dr
c       px=l*dp
c     do ie=1,nnn*mstc(5)
c       i=iofset+ie
c       rr=(r(1,i)-x)**2+r(2,i)**2+r(3,i)**2
c       pp=((p(1,i)-px)**2+p(2,i)**2+p(3,i)**2)/hc
c        rhor(l)=rhor(l)+exp(-rr/widr)/facr
c        rhop(l)=rhor(l)+exp(-pp/widp)/facp
c     end do
c     end do
      ...
      write(12,800)rr,histr(i)/nev,rhor(i)/nev,
     &      pp,histp(i)/nev,rhop(i)/nev
      end do
      close(12)

800   format(2(f8.3,1x,e15.8,1x,e15.8))
      end
...
c...Phys. Rev. C80 054903 (2009)

      implicit double precision(a-h, o-z)
      include 'jam2.inc'
      dimension rot(3,3),pr(3)

c...generic parameterization of radius and surface thickness
      rad=1.12*ia**0.333333-0.86/ia**0.333333
      dr = 0.54
      density0 = 0.17

c...explicit values for specific nuclei
c...Taken from C.W.De Jager et al.,
c...Atom.Data.Nucl.Data Tabl.36, 495 (1987).
c...Deformation pameters taken from Moller et al., 
c..Atomic Data and Nuclear Data Tables, 59, 185-381, (1995). 
      if(ia.eq.197) then  ! Au
        rad = 6.38
        dr = 0.535
        density0 = 0.1695
        beta2 = -0.13
        beta4 = -0.03
      else if(ia.eq.208 .or. ia.eq.207) then  ! Pb
        rad = 6.62
        dr = 0.546
        density0 = 0.1695
        beta2 = 0.0;
        beta4 = 0.0;
      else if(ia.eq.63) then  ! Cu
        rad = 4.20641
        dr = 0.5877
        density0 = 0.1686
        beta2 = 0.162;
        beta4 = 0.006;
      else if(ia.eq.238) then ! U
        beta2 = 0.28
        beta4 = 0.093
      else 
       call jamerrm(30,1,'(jamdefrm:) mass number not available'
     &       //' for deformation paraameter')
       stop
      endif
...
10    continue
cc...Uniform distribution in a sphere with r = rmaxCut
      r = rmaxCut*rn(0)**(1.0/3.0)
      cx = 1.0-2.0*rn(0)
    
c...deformation Main axis in z-axis
c...Spherical Harmonics Y^0_2
      y20 = sqrt(5.0/16.0/paru(1))*(3.0*cx*cx-1.0)
c...Spherical Harmonics Y^0_4
      y40 = 35.0*cx*cx*cx*cx - 30.0*cx*cx + 3.0
      y40 = y40 * 3.0/16.0/sqrt(paru(1))
      rad1 =rad*(1.0 + beta2 * y20 + beta4 * y40);
      rwMax1 =1.0/(1.0+exp(-rad1/dr));
      if(rn(0)*rwMax1 .gt. 1.0/(1.0+exp((r-rad1)/dr))) goto 10
...
c....side-side Rz(pi/2)*Ry(pi/2)
      ...
      pr(2)=y
      pr(3)=z
      x=rot(1,1)*pr(1)+rot(1,2)*pr(2)+rot(1,3)*pr(3) 
      y=rot(2,1)*pr(1)+rot(2,2)*pr(2)+rot(2,3)*pr(3) 
      z=rot(3,1)*pr(1)+rot(3,2)*pr(2)+rot(3,3)*pr(3) 
...
c...Purpose: to set default for decay of particles.
      ...
      include 'jam2.inc'
        ...
        mdcy(jamcomp(111),1)=is  ! pi0
        mdcy(jamcomp(311),1)=is  ! k0
        mdcy(jamcomp(-311),1)=is ! ak0
        mdcy(jamcomp(411),1)=is    ! D+
        mdcy(jamcomp(-411),1)=is   ! D-
        mdcy(jamcomp(421),1)=is    ! D0
        mdcy(jamcomp(-421),1)=is   ! aD0
        mdcy(jamcomp(221),1)=is    ! eta
c       mdcy(jamcomp(331),1)=is    ! eta'
        mdcy(jamcomp(441),1)=is    ! eta_c

        mdcy(jamcomp(310),1)=is    ! K_S0
        mdcy(jamcomp(431),1)=is    ! D_s+
        mdcy(jamcomp(-431),1)=is   ! D_s-
        mdcy(jamcomp(511),1)=is    ! B0
        mdcy(jamcomp(-511),1)=is   ! B0~
        mdcy(jamcomp(521),1)=is    ! B+
        mdcy(jamcomp(-521),1)=is   ! B-
        mdcy(jamcomp(531),1)=is    ! B_s0
        mdcy(jamcomp(-531),1)=is   ! B_s0~

        mdcy(jamcomp(3122),1)=is   ! Lambda0
        mdcy(jamcomp(-3122),1)=is
        mdcy(jamcomp(3112),1)=is   ! Sigma-
        mdcy(jamcomp(-3112),1)=is
        mdcy(jamcomp(3212),1)=is   ! Sigma0
        mdcy(jamcomp(-3212),1)=is
        mdcy(jamcomp(3222),1)=is   ! Sigma+
        mdcy(jamcomp(-3222),1)=is

        mdcy(jamcomp(3312),1)=is   ! Xi-
        mdcy(jamcomp(-3312),1)=is
        mdcy(jamcomp(3322),1)=is   ! Xi0
        mdcy(jamcomp(-3322),1)=is

        mdcy(jamcomp(3334),1)=is   ! Omega-
        mdcy(jamcomp(-3334),1)=is

        mdcy(jamcomp(4112),1)=is   ! Sigma_c0
        mdcy(jamcomp(4122),1)=is   ! Lambda_c+
        mdcy(jamcomp(4212),1)=is   ! Sigma_c+
        mdcy(jamcomp(4222),1)=is   ! Sigma_c++
        mdcy(jamcomp(-4112),1)=is  ! aSigma_c0
        mdcy(jamcomp(-4122),1)=is  ! aLambda_c+
        mdcy(jamcomp(-4212),1)=is  ! aSigma_c+
        mdcy(jamcomp(-4222),1)=is  ! aSigma_c++

        mdcy(jamcomp(4132),1)=is   ! Xi_c0
        mdcy(jamcomp(4312),1)=is   ! Xi'_c0
        mdcy(jamcomp(4232),1)=is   ! Xi_c+
        mdcy(jamcomp(4322),1)=is   ! Xi'_c+
        mdcy(jamcomp(4332),1)=is   ! Omega_c0
        mdcy(jamcomp(-4132),1)=is  ! aXi_c0
        mdcy(jamcomp(-4312),1)=is  ! aXi'_c0
        mdcy(jamcomp(-4232),1)=is  ! aXi_c+
        mdcy(jamcomp(-4322),1)=is  ! aXi'_c+
        mdcy(jamcomp(-4332),1)=is  ! aOmega_c0

        mdcy(jamcomp(4412),1)=is   ! Xi_cc+ 
        mdcy(jamcomp(-4412),1)=is  ! Xi_cc+~ 
        mdcy(jamcomp(4422),1)=is   ! Xi_cc++
        mdcy(jamcomp(-4422),1)=is  ! Xi_cc--~ 
        mdcy(jamcomp(4432),1)=is   ! Omega_cc+
        mdcy(jamcomp(-4432),1)=is  ! Omega_cc-~ 

        mdcy(jamcomp(5112),1)=is   ! Sigma_b-
        mdcy(jamcomp(5122),1)=is   ! Lambda_b0
        mdcy(jamcomp(5212),1)=is   ! Sigma_b0
        mdcy(jamcomp(5222),1)=is   ! Sigma_b+
        mdcy(jamcomp(-5112),1)=is  ! aSigma_b-
        mdcy(jamcomp(-5122),1)=is  ! aLambda_b0
        mdcy(jamcomp(-5212),1)=is  ! aSigma_b0
        mdcy(jamcomp(-5222),1)=is  ! aSigma_b+
        mdcy(jamcomp(5232),1)=is   ! Xi_b0
        mdcy(jamcomp(-5232),1)=is  ! Xi_b0~

        mdcy(jamcomp(5142),1)=is   ! Xi_bc0
        mdcy(jamcomp(-5142),1)=is  ! Xi_bc0~
        mdcy(jamcomp(5242),1)=is   ! Xi_bc+
        mdcy(jamcomp(-5242),1)=is  ! Xi_bc-~
        mdcy(jamcomp(5312),1)=is   ! Xi_'b-
        mdcy(jamcomp(-5312),1)=is  ! Xi_'b+~
        mdcy(jamcomp(5322),1)=is   ! Xi_'b0
        mdcy(jamcomp(-5322),1)=is  ! Xi_'b0~

        mdcy(jamcomp(5332),1)=is   ! Omega_b-
        mdcy(jamcomp(-5332),1)=is  ! Omega_b+~

        mdcy(jamcomp(5342),1)=is   ! Omega_bc0
        mdcy(jamcomp(-5342),1)=is  ! Omega_bc0~
        mdcy(jamcomp(5412),1)=is   ! Xi'_bc0
        mdcy(jamcomp(-5412),1)=is  ! Xi'_bc0~
        mdcy(jamcomp(5422),1)=is   ! Xi'_bc+
        mdcy(jamcomp(-5422),1)=is  ! Xi'_bc-~
        mdcy(jamcomp(5432),1)=is   ! Omega'_bc0
        mdcy(jamcomp(-5432),1)=is  ! Omega'_bc0~
        mdcy(jamcomp(5442),1)=is   ! Omega'_bcc+
        mdcy(jamcomp(-5442),1)=is  ! Omega'_bcc-~

        mdcy(jamcomp(5512),1)=is   ! Xi_bb-
        mdcy(jamcomp(-5512),1)=is  ! Xi_bb+~
        mdcy(jamcomp(5522),1)=is   ! Xi_bb0
        mdcy(jamcomp(-5522),1)=is  ! Xi_bb0~
        mdcy(jamcomp(5532),1)=is   ! Omega_bb-
        mdcy(jamcomp(-5532),1)=is  ! Omega_bb+~
        mdcy(jamcomp(5542),1)=is   ! Omega_bbc0
        mdcy(jamcomp(-5542),1)=is  ! Omega_bbc0~
...
c....In case projectile is a hadron.
      if(mstd(2).eq.1) then
       kfproj=mstd(1)
       ihnt2(5)=kfproj
...
c...The rest mass of projectile particle mass
        hint1(8)=pard(34)
...
c...Strangeness.
        ...
        if(kf.eq.92) then  ! strings
          jamk=kfprop(kq(1,i),3)+kfprop(kq(2,i),3)
c         jamk=kchg(kc1,7)*isign(1,kf1)+kchg(kc2,7)*isign(1,kf2)
        else if(k(1,i).eq.4) then ! partons
          jamk=kfprop(k(2,i),3)
        else if(abs(kf).gt.1000000000) then
          jamk=0
        else
          kc=jamcomp(kf)
...
c...Read out charge and change sign for antiparticle.
      ...
      if(abs(kf).gt.1000000000) jamchge=mod(abs(kf)/1000,1000) 
...
c***********************************************************************
      subroutine jamdxw
c...Add small width (1 MeV) to D*, B*
c***********************************************************************
      implicit double precision(a-h, o-z)
      include 'jam2.inc'
      parameter(hc=0.197327d0)
      parameter(mxadd=11)
c     common/pjdat2/kchg(500,7),pmas(500,4),parf(2000),vckm(4,4)
      dimension kfdx(mxadd)
      data kfdx/413,423,433,443,10433
     &,4114,4214,4224,4314,4324,4334/
c... B* (513,523,533,...) decays in EM int. ignore them
c 513,523,533,543,553,
c...[ AOtest
c	iiii=jamcomp(413)
c	write(*,*) 'mdcy(413)=',iiii,(mdcy(iiii,k),k=1,3)
c...] AOtest
c tau=hc/gamma=0.197/gamma(in GeV) (fm)=0.197*10^{-6}/gamma(in GeV) (nm)
      do i=1,mxadd
        kf=kfdx(i)
        kc=jamcomp(kf)
        pmas(kc,2)=1.0d-3   ! 1 MeV
        pmas(kc,4)=paru(3)/pmas(kc,2)*1.0d-6
c	write(*,*) kf,kc
      enddo
      mdcy(jamcomp(513),1)=0
      mdcy(jamcomp(523),1)=0
      end

c***********************************************************************

      subroutine jamrmspectator

c....Remove spectotor nculeons which will not collide.
      implicit double precision(a-h, o-z)
      include 'jam1.inc'
      include 'jam2.inc'
      parameter(mxchan=30)
...
c...Get collision number.
      ...
      ftime=parc(2)*mstc(3)
      mentry=0
      do 100 jt=1,mstd(5)  ! loop over projectile
      do 110 jp=1+mstd(5),mstd(2)+mstd(5)  ! loop over target
        kf1=k(2,jp)
        kf2=k(2,jt)
        ibar1=k(9,jp)
        ibar2=k(9,jt)
...
c       icoll(1,mentry)=jp
c       icoll(2,mentry)=jt
c       coll(1,mentry)=(tcol1+tcol2)/2d0
c       coll(2,mentry)=tcol1
c       coll(3,mentry)=tcol2
c       coll(4,mentry)=sig
c       coll(5,mentry)=sigel
c       coll(6,mentry)=b2
  ...
  100 continue
      ...
      do i=1,nv
c...propagate spectator.
        if(k(6,i).eq.0) then
         r(1,i)=r(1,i)+p(1,i)/p(4,i)*ftime
         r(2,i)=r(2,i)+p(2,i)/p(4,i)*ftime
         r(3,i)=r(3,i)+p(3,i)/p(4,i)*ftime
         endif
         k(6,i)=0
      end do
