c...A main program for Au(20GeV/c)+Au
...
c     bmax=-4.0d0         ! maximum impact parameter 7.2% NA49
      dt=50.0
      nstep=1
      mstc(6)=101         ! RQMD/S mode
      if(mstc(6).eq.101) then
        dt=0.1d0            ! time step size (fm/c)
        nstep=400           ! total number of timestep
        mstc(106)=5         !=1 mh, =2 mm, =3 ms, =4 ah, =5 as  par. set
        mstc(107)=0         !=0 mom+sky(mh,mm,ms), =1 sky(ah,as), =2 cut mom+sky
        mstc(103) =1        ! RQMD/S t-development approx.
      endif
      cwin='2.0gev'      ! incident energy
      frame='nn      '    ! comp. frame
...
c     targ='208Pb'         ! target
      mstc(8)=0    ! job mode.
      mstc(165)=1  !  time evolution of particle
      mstc(156)=1  !  time evolution of collision
      mstc(167)=1  !  time evolution of density
c     mstc(77)=1  !   d pi
c     mstc(78)=0  !   d pi pi
