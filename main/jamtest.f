...
c***********************************************************************
      ...
      implicit double precision(a-h, o-z)
      include 'jam2.inc'
      dimension sig1(11),sig0(11),sigin(30),sigin1(9),sigin2(30)
      parameter ( emnuc = 0.93827d0, empion = 0.138d0 )
      parameter(nchnl=12)
...
c     ds=0.1d0
      ...
      do i=0,nnn
        ...
        call jamxnnin(srt,sig1,ipn,ideut)
        if(ipn.eq.2) call jamxnnin(srt,sig0,0,ideut)
     ...
     $                 sig0,sigel0,sigin2,mchanel,mabsrb,ijet,icon)
         sigint3=0.0d0
         do ic=1,mchanel
...
c       srt0=max(10,srt)
        ...
        sigd2=0
        if(srt.gt.3.0d0) then
        sigd2=10.5372d0*(srt-3.0d0)**1.06245d0
     &        /(15.2027d0+(srt-9.43717d0)**2)/2.d0
        sigd2=2*sigd2
        endif

C...Single diffractive scattering cross-section from Goulianos:
       s=srt*srt
       sigsd=2.*0.68*(1.+36./s)*log(0.6+0.1*s)


            if(ipn.eq.1) then
            sigd3=0.25*(sig1(1)+sig1(4))+sig1(2)
            sigd3=2*sigd3
          sigin(1)=sig1(1)            ! ND
          ...
          sigin(9)=sig1(9)            ! D*D*
            ...
            sigd3=sig1(1)+0.5*(sig1(2)+sig0(2))+0.5*sig1(4)
            sigd3=2*sigd3
...
c       write(6,811)srt,plab,sigint,sigint3,sigint2,sigstr,
c    &  sigd,sigd2,sigd3,sigsd

        write(6,811)srt,plab,sigint,sigint3,sigint2,sig,sigel
...
c     parameter(smax=2.5,ds=0.001 )
      parameter(smax=4.,ds=0.01 )
c     parameter(smax=30.d0,ds=0.02d0 )
      dimension sigt(0:6,0:6,0:5)
      logical liner
c...Functions: momentum in two-particle cm.
      pawt(a,b,c)=sqrt((a**2-(b+c)**2)*(a**2-(b-c)**2))/(2.d0*a) 
...
c...log
     ...
     $                 sig,sigel,sigin,mchanel,mabsrb,ijet,icon)
        if(msel.eq.2)
     $  call jamcross(2,icltyp,srt,pr,kf1,kf2,em1,em2,
     $                 sig2,sigel2,sigin,mchanel,mabsrb,ijet,icon)
...
c...sigt(0,0,5) non diffractive
        ...
        if(msel.eq.1) then
         call jamsighh(sigt3,24,srt)
         call jamsighh(sige3,25,srt)
          sigint=sig-sigel
c         write(60,810)srt,plab,sig,sigel,sigint,sigd,sigdd
          write(60,810)srt,plab,sig,sigel,sigint,sigt3,sige3
        else
          sigintot=0.0d0
          do j=1,mchanel+mabsrb
           sigintot=sigintot+sigin(j)
          enddo

          write(60,810)srt,plab,sig,sigel,sigintot
     $           ,(sigin(j),j=1,mchanel+mabsrb)

c         call xsbw1(0,srt,pr,kf1,kf2,iz1,iz2,sigr,2)
c         write(60,815)mchanel,mabsrb,
c    $          srt,pr,siginttot,(sigin(j),j=1,2),sigr
c    $          srt,pr,siginttot,(sigin(j),j=1,mchanel+mabsrb)
...
c...Akaon-N total/background elastic.
        call jamxkp(kf1,kf2,srt,pr,em1,em2,sigb,sigelb,sigchb,sigy)

c       print *,'after jamxkp',kf1,kf2,srt,sig1,sigelb,sigchb,sigy

c...Breit-Wigner cross sections.
        call jamxbw1(istr,srt,pr,kf1,kf2,iz1,iz2,sigr,sige,2)
c       print *,'after jamxbw1',kf1,kf2

        call jamcross(1,icltyp,srt,pr,kf1,kf2,em1,em2,
     $                 siga,sigela,sigin,mchanel,mabsrb,ijet,icon)

c       print *,'after jamcross',kf1,kf2
c       read(5,*)
...
c  4: k- n => sigma0 pi-
     ...
     $ ,sigbwp(5)+sigy(4),sigy(4),sigbwp(5)
      ...
      write(61,810)srt,plab,sigtt,sigeltt,sigr,sige
     $     ,siga,sigela,sigb,sigelb
      
        write(60,810)srt,plab,siga,sigela
...
c...Breit-Wigner cross sections.
     ...
     $                 sig,sigel,sigin,mchanel,mabsrb,ijet,icon)
         ...
         print *,srt,plab,sig,sigel

        call jamcross(2,icltyp,srt,pr,kf1,kf2,em1,em2,
     $                 sig,sigel,sigin,mchanel,mabsrb,ijet,icon)

      sss=sigin(1)+sigin(2)+sigin(3)
      write(55,810)srt,plab,sig-sigel,sss,sigin(1),sigin(2),sigin(3)
c...piN cross section
...
c     common/pjjets/n,npad,k(1000,5),p(1000,5),v(1000,5)
...
c     p(1,5)=pjmass(kf)
      ...
      p(1,5)=1.57826712971068
      p(1,4)= sqrt(p(1,5)**2+p(1,1)**2+p(1,2)**2+p(1,3)**2)
c     call jamrdec(icon,iang,isang,gpt) 
      call pjdecy(1,icon)
      call pjlist(1)
      if(icon.ne.0) then
        print *,'icon=',icon 
...
c     mstc(63)=1 ! Frankfurt
c     mstc(63)=4 ! 

c     msel=2  ! mass integral itself
      msel=1  ! if you want to compute cross section
      emnuc=0.938d0
c     em1=1.18d0
      em1=1.232d0
      em2=0.938d0
...
800   format(//,'# ',i5,1x,a16,1x,f8.5,' + ',i5,1x,a16,1x,f8.5)
      ...
      do i=0,nn
       srt=s0+i*ds
        call jamxnnin(srt,sigin,1,1)
        sig=0.75*sigin(1)    ! pp -> nd++ cross section
c       sig=0.25*sigin(1)    ! pp -> pd+ cross section
        pr=pawt(srt,em1,em2)
        pr2new=0.25*srt*srt-emnuc**2

c....Giessen
        mstc(62)=31
        call jamdetb1(msel,kc1,srt,em1,em2,pr,db1,nhlf)
...
c...Danielewicz
        mstc(62)=33
        call jamdetb1(msel,kc1,srt,em1,em2,pr,db3,nhlf)
c
c...Naive detbal
        sig1=fac*sig*pr2new/(pr*pr)
        sig2=fac*sig*db1*pr2new
        sig3=fac*sig*db2*pr2new
        sig4=fac*sig*db3*pr2new
        print *,srt-em1,sig1,sig2,sig3,sig4
c       print *,srt-em1,sig3
        write(11,810)srt,db2
