...
c     mstc(38)=6
c     hipr1(44)=2.0
c     parj(2)=0.33    ! P(s)/P(q)
c     mstc(74)=0        ! dipole radiaton
c     mstc(61)=2  ! Resonance decay angular distribution 0: isotropic
...
c     bmax=-3.0d0         ! maximum impact parameter
c     bmax=-3.338d0         ! maximum impact parameter
      bmax=-3.4d0         ! maximum impact parameter 5.0% NA49 158GeV
c     bmax=-4.0d0         ! maximum impact parameter 7.2% NA49

      dt=100.0d0          ! collision time(fm/c)
      nstep=1
c     cwin='200gevc        '  ! incident energy
c     cwin='200gev        '  ! incident energy
c     cwin='0.4gev         '  ! incident energy
c     cwin='2.0gev        '  ! incident energy
c     cwin='4.0gev        '  ! incident energy
c     cwin='6.0gev        '  ! incident energy
c     cwin='8.0gev         '  ! incident energy
      cwin='10.7gev         '  ! incident energy
c     cwin='20.0gev         '  ! incident energy
c     cwin='30.0gev         '  ! incident energy
c     cwin='40.0gev         '  ! incident energy
c     cwin='80.0gev         '  ! incident energy
c     cwin='158.0gev        '  ! incident energy
c     cwin='10.0gev        '  ! incident energy
c     cwin='52.35gev        '  ! incident energy
c     frame='collider'        ! comp. frame
      frame='nn      '        ! 0omp. frame
c     proj='p'         ! projectile
c     targ='p'         ! target
c     mstc(17)=1  !   only inelastic collision
c     proj='32S'         ! projectile
c     targ='32S'         ! target
c     proj='2H'         ! projectile
c     targ='2H'         ! target
...
c     targ='208Pb'         ! target
      mstc(8)=0    ! job mode.
c     mstc(74)=1   ! dipole-approximated QCD radiation of the string
c     mstc(81)=0   ! 1:hard scattering on/off
c     mstc(156)=1  ! analysis of collision distribution
c     mstc(63)=0  ! frozen delta
c     mstc(165)=1  !  time evolution of particle
c     mstc(156)=1  !  time evolution of collision
c     mstc(167)=1  !  time evolution of density
c     mstc(77)=1  !   d pi
c     mstc(78)=0  !   d pi pi
c     mstc(45)=1  !   deuteron coalescence
c     parc(8)=55
c     mstc(63)=2  !   delta decay width

c     call xxx(cwin,proj,targ,frame)

c     mstc(42)=0 ! allow weak decay after simulation.
c     mdcy(jamcomp(111),1)=1    ! pi0
      mdcy(jamcomp(311),1)=1    ! k0
      mdcy(jamcomp(-311),1)=1   ! ak0 
      mdcy(jamcomp(310),1)=1    ! k0_S
      mdcy(jamcomp(130),1)=1    ! k0_S

c     mdcy(jamcomp(3122),1)=1   ! Lambda0
c     mdcy(jamcomp(-3122),1)=1
c     mdcy(jamcomp(3112),1)=1   ! Sigma-
c     mdcy(jamcomp(-3112),1)=1
c     mdcy(jamcomp(3212),1)=1   ! Sigma0
c     mdcy(jamcomp(-3212),1)=1
c     mdcy(jamcomp(3222),1)=1   ! Sigma+
c     mdcy(jamcomp(-3222),1)=1
c     mdcy(jamcomp(3312),1)=1   ! Xi-
c     mdcy(jamcomp(-3312),1)=1
c     mdcy(jamcomp(3322),1)=1   ! Xi0
c     mdcy(jamcomp(-3322),1)=1
...
c           write(33,802) k(2,i),p(5,i),(p(j,i),j=1,3),(v(j,i),j=1,4)
            write(33,802) k(2,i),p(5,i),(p(j,i),j=1,4)
          end do
        endif
...
c....Rapidity distribution.
      ...
      if(pard(16).le.4.d0) then
        wy=0.10d0
        nymx=(ymax-ymin)/wy
      else if(pard(16).le.10.d0) then
        wy=0.15d0
        nymx=(ymax-ymin)/wy
      else if(pard(16).le.40.d0) then
        wy=0.25d0
c       wy=0.2d0
        nymx=(ymax-ymin)/wy
      else
...
c     endif
      ...
      call vbook1(14,'dN/dy - pion0 ',nymx,ymin,ymax)
      ...
      call vbook1(18,'dN/dy - charged',nymx,ymin,ymax)
      call vbook1(19,'dN/dy - K-',nymx,ymin,ymax)
      call vbook1(20,'dN/dy - K+',nymx,ymin,ymax)


c...Transverse distributions.

      if(pard(16).le.10.d0) then
        pmin=0.0d0
        pmax=1.0d0
        npmx=30
      else if(pard(16).le.40.d0) then
      ...
      call vbook1(24,'dN/dpt**2 - pion0  ',npmx,pmin,pmax)
      call vbook1(25,'dN/dpt**2 - charged',npmx,pmin,pmax)
      call vbook1(26,'dN/dp - pi0',npmx,pmin,pmax)
...
c       if(mstc(4).eq.0) then  ! lab. fram
c       else if(mstc(4).eq.3) then  ! collider
c       else
c        rap=rap+ylab
c       endif
...
c...Charged particles.
        if(kch.ne.0) then
          nch=nch+1
          npa(1)=npa(1)+1
          call vfill1(18,rap,1.d0/wy)
          call vfill1(31,eta,1.d0/wy)
          call vfill1(25,pt,1.d0/wp/pt/2) 
...
c.......Pions.
          ...
          call vfill1(20,rap,1.d0/wy) 
        else if(kf.eq.-321) then
          call vfill1(19,rap,1.d0/wy) 
